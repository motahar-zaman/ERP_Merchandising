            ...
            @if($cost_breakdown_fabric->count() >1)
                @foreach($cost_breakdown_fabric as $fabric)
                        <tr id="product{{$num}}">
                            ...
                            <td>
                                <div class="form-group {{ $errors->has('fabric_content') ? 'has-error' : '' }}">
                                    {{--{{ Form::label('','Unit : ') }}--}}
                                    {{ Form::text('fabric_content'.$num,$fabric ? $fabric->fabric_content:null ,['id'=>'fabric_content1','class'=>'form-control','placeholder'=>'Fabric Content']) }}
                            ...
                            <td>
                                <div class="form-group {{ $errors->has('fabric_description'.$num) ? 'has-error' : '' }}">
                                    {{--{{ Form::label('','Unit : ') }}--}}
                                    {{ Form::text('fabric_description'.$num,$fabric ? $fabric->fabric_description:null,['id'=>'fabric_description'.$num,'class'=>'form-control','placeholder'=>'Description']) }}
                            ...
                            <td>
                                <div class="form-group {{ $errors->has('fabric_cost'.$num) ? 'has-error' : '' }}">
                                    {{--{{ Form::label('','Unit : ') }}--}}
                                    {{ Form::text('fabric_cost'.$num,$fabric ? $fabric->fabric_cost:null,['id'=>'fabric_cost'.$num,'class'=>'form-control','placeholder'=>'Appointed Trims Cost US$ With DZN ']) }}
