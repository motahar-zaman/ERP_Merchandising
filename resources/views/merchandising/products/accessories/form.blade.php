...
<div class="form-group{{ $errors->has('accessories_category_id') ? 'has-error' : '' }}">
    {{ Form::label('accessories_category_id', 'Category', ['class'=>'label-control']) }}
    {{ Form::select('accessories_category_id',$repository->accessoriesCategories(),null,['class'=>'form-control populate select2','placeholder'=>'Select Category','required']) }}
    @if ($errors->has('accessories_category_id'))
        <span class="text-danger">
                <p>{{ $errors->first('accessories_category_id') }}</p>
            </span>
    @endif
</div>
...
<div class="form-group{{ $errors->has('description') ? 'has-error' : '' }}">
    ...
    {{ Form::textarea('description',null,['class'=>'form-control','placeholder'=>'','rows'=>5,'cols'=>5]) }}
