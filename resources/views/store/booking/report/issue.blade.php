...
@section('content')
  ...
  <section class="sheet padding-10mm" style="">
   {{--  <div id="logo" style="float:right; display: inline;">
    ...
    <center>
      <h1>{{-- @if($data == 'daily') Daily @elseif($data == 'weekly') Weekly @else Monthly @endif --}} Issue/Supply Report</h1>
    </center>
    <br><br>
    ...
    <table class="table" align="center" cellpadding="0" cellspacing="0" border="0" style="margin-top:20px;">
         <thead>
            <tr style="font-size: 13px;height: 30px; background-color: #e3e1dc;" class="text-center">    
                ...
                <th class="td text-center"> {{-- @if($data == 'daily') Today @elseif($data == 'weekly') Last 1 Week @else Last 1 Month @endif --}}Date Wise Issued </th>
                <th class="td text-center"> Total Issued</th>
                <th class="td text-center"> In Hand Qty</th>
            ...
            @foreach($store_booking_details as $store_booking_detail)
              <tr>
                 ...
                 <td class="td text-right">
                    @php
                    $today_issued = $store_booking_detail->store_inventory_details
                                  ...
                                  ->when($from_date != null && $to_date != null, function ($query) use ($to_date, $from_date){
                                      return $query->where('issue_date', '>=', $from_date)->where('issue_date', '<=', $to_date);
                                  })->sum('issued');
                    // $date_today = date('Y-m-d');
                    // if($data == 'daily'){
                    //   $today_issued = $store_booking_detail->store_requisition_details->where('issue_date', $date_today)->sum('received_qty');
                    // }elseif ($data == 'weekly') {
                    //     $last_seven_date = date('Y-m-d', strtotime('-7 day'));
                    //     $today_issued = $store_booking_detail->store_requisition_details->where('issue_date', '<=', $date_today)->where('issue_date', '>=', $last_seven_date)->sum('received_qty');
                    // }else{
                    //     $last_thirty_date = date('Y-m-d', strtotime('-1 month'));
                    //     $today_issued = $store_booking_detail->store_requisition_details->where('issue_date', '<=', $date_today)->where('issue_date', '>=', $last_thirty_date)->sum('received_qty');
                    // }
                    @endphp
                    {{ $today_issued }}
                  </td>
                  <td class="td text-right">{{($store_booking_detail->store_requisition_details->sum('issued'))}}</td>
