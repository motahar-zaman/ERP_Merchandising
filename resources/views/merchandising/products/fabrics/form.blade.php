...
<div class="form-group{{ $errors->has('fabric_category_id') ? 'has-error' : '' }}">
    {{ Form::label('fabric_category_id', 'Category', ['class'=>'label-control']) }}
    {{ Form::select('fabric_category_id',$repository->fabricCategories(),null,['class'=>'form-control populate select2','placeholder'=>'Select Category','required']) }}
    @if ($errors->has('fabric_category_id'))
        <span class="text-danger">
                <p>{{ $errors->first('fabric_category_id') }}</p>
        </span>
    @endif
</div>
