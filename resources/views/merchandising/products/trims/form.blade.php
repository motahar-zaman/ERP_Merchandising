...
<div class="form-group{{ $errors->has('trims_category_id') ? 'has-error' : '' }}">
    {{Form::label('trims_category_id', 'Category', ['class'=>'label-control'])}}
    {{ Form::select('trims_category_id',$repository->trimsCategories(),null,['class'=>'form-control populate select2','placeholder'=>'Select Category','required']) }}
    @if ($errors->has('trims_category_id'))
