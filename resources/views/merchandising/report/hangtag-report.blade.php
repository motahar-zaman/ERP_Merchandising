...
@section('title','All Tag Booking Report')
<style>
    .sig-hr{
        font-size: 20px;
        font-weight: 600;
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">All Tag Booking Report</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-10 text-center">
                            <h2>{{$hangtag_bookings->unit_id != null ? $hangtag_bookings->CompanyUnit->name : ''}}</h2>
                            <h4>All Tag Booking Report </h4>
                        </div>
                            ...
                            <table id="datatable_buttons" class="table table-hover table-bordered" style="line-height: .1">
                                <thead>
                                    <tr>
                                        ...
                                        <td class="text-center">{{$hangtag_bookings->date}}</td>
                                    ...
                                    <tr>
                                        <th colspan="3">To</th>
                                        <td class="text-center">{{$hangtag_bookings->to}}</td>
                                        <th colspan="3"> A-6(part),7-8(part),BSCIC Industrial State </th>
                                        <th class="text-center"></th>
                                    </tr>
                                    <tr>
                                        <th colspan="3">Attn</th>
                                        <td class="text-center">{{$hangtag_bookings->attn}}</td>
                                    ...
                                    <tr>
                                        <th colspan="3">Sub</th>
                                        <td class="text-center">{{$hangtag_bookings->sub}}</td>
                                ...
                                <tr>
                                    ...
                                    @foreach($hangtag_booking_details as $hangtag_bookings_detail)
                                        <tr>
                                            <td class="text-center">{{$hangtag_bookings_detail->desc}}</td>
                                            <td class="text-center">{{$hangtag_bookings_detail->color}}</td>
                                            <td class="text-center">{{$hangtag_bookings_detail->upc}}</td>
                                            <td class="text-center">{{$hangtag_bookings_detail->retail_price}}</td>
                                            <td class="text-center">{{$hangtag_bookings_detail->size}}</td>
                                            <td class="text-center">{{$hangtag_bookings_detail->gmnts_qty}}</td>
                                            <td class="text-center">{{$hangtag_bookings_detail->percentage}}</td>
                                            <td class="text-center">{{$hangtag_bookings_detail->book_qty}}</td>

                                            @php $gmnts_qty_subtotal+=$hangtag_bookings_detail->gmnts_qty; @endphp
                                            @php $retail_subtotal+=$hangtag_bookings_detail->retail_price; @endphp
                                            @php $book_qty_subtotal+=$hangtag_bookings_detail->book_qty; @endphp
...
@section('script')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#cmd').click(function () {
                var doc = new jsPDF();
                var date = new Date();
                doc.text(90, 10, 'All Tag Booking');
                doc.autoTable({html: '#datatable_buttons',
                    theme : 'striped',
                    styles: {fontSize: 7}
                });
                doc.save('.All_Tag_Booking');
            });
        });
    </script>
    ...
    <script>
        function exportTableToExcel(datatable_buttons,test){
            ...
            // Specify file name
            test1 = test?test+'.xls':'Hangtag_Booking.xls';
